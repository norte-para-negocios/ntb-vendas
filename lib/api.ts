...
export const fetchDiscoveredPrinters = async (storeId: string): Promise<{ name: string; machine: string; kind: string }[]> => {
  const { data, error } = await supabase.from('discovered_printers').select('name, machine, kind').eq('store_id', storeId).order('name', { ascending: true });
  if (error) { console.error('Error fetching discovered printers:', error); return []; }
  return (data || []).map((row) => ({ name: row.name, machine: row.machine || '', kind: row.kind || 'system' }));
};
