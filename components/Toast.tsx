...
export function ToastViewport() {
  ...
  return (
    <div
      role="status"
      aria-live="polite"
      className="fixed top-[max(1rem,env(safe-area-inset-top))] right-4 md:top-auto md:bottom-4 z-[100] flex flex-col gap-2 w-[calc(100%-2rem)] max-w-sm pointer-events-none"
    >
      {items.map((t) => {
        ...
        return (
          ...
          >
            ...
            <button onClick={() => dismiss(t.id)} className="relative hit-44 text-[var(--text-muted)] hover:text-[var(--text)] u-motion shrink-0">
              <X size={14} />
            </button>
          </div>
        );
